...
def train(epoch, model, optimizer, criterion, train_loader, run_config,
    ...
    # old way to notify this is the training process
    model.train()
    ...
    for step, (data, targets) in enumerate(train_loader):
        ...
        data = data.cuda()  # load all data to GPU
...
def test(epoch, model, criterion, test_loader, run_config, writer):
    ...
    # old way to notify this is the testing process
    # Now pp often use: with torch.no_grad():
    model.eval()
...
def main():
    ...
    # lấy thông tin config cho model ResNeXt từ user thông qua terminal 
    config = parse_args()
    ...
    # model
    model = load_model(config['model_config']) # loading resNeXt model
    model.cuda()  # activate GPU
    n_params = sum([param.view(-1).size()[0] for param in model.parameters()])
    logger.info('n_params: {}'.format(n_params))

    criterion = nn.CrossEntropyLoss(size_average=True)  # Hàm đánh giá : Cross Entropy loss 

    # optimizer
    optimizer = torch.optim.SGD(      # dùng SGD để điều chỉnh grad 
        model.parameters(),
    ...
    for epoch in range(1, optim_config['epochs'] + 1):
        ...
        model_path = os.path.join(outdir, 'model_', str(state['accuracy']) ,'_state.pth')
